...
One shared component template renders **65 client sites across 65 verticals**, each
...
deterministic **QA harness** (`site/qa/`, `npm run qa:all`) sweeps all 65 sites
...
- **`/sites`** — portfolio index of all 65 generated sites.

All 65 come from one template; only `content/clients/<slug>/copy.json` (bilingual),
...
they are build- and offline-safe. Gallery prompts are derived from each site's own
