      ...
      <div className="grid auto-rows-[160px] grid-cols-2 gap-3 sm:auto-rows-[180px] sm:grid-cols-6">
        {TILES.map((tile, i) => (
          ...
          >
            ...
            {/* Caption always readable (touch/keyboard); scrim intensifies on hover/focus */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/15 to-transparent transition-colors duration-300 group-hover:from-black/85 group-focus-within:from-black/85" />
            <figcaption className="absolute inset-x-0 bottom-0 p-4 font-display text-sm font-semibold italic leading-tight text-white">
              {captions[i] ?? ""}
            </figcaption>
          </Reveal>
